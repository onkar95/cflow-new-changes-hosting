...
import Mytendor from "../Mytendor/Mytendor";
...
function HomeUser({ setIsUser, isSignup, setIsSignup }) {
  ...
  const [temp_filter_pitched_requests, setTemp_filter_pitched_requests] =
    useState([]);
  const [temp_filter_saved_requests, setTemp_filter_saved_requests] = useState(
    []
  );
  ...
  const [currentSectionTendor, setCurrentSectionTendor] = useState(0);
  ...
  const getPitchedRequests = async () => {
    await axios
      ...
      .then(function (response) {
        ans = response?.data?.data;
        //ans=ans.filter(o=>o.product_status!=="rejectedPitch")
        ans = ans.reverse();
      });
    setPitchedRequests(ans.filter((o) => o.product_status !== "rejectedPitch"));
    setTemp_filter_pitched_requests(
      ans.filter((o) => o.product_status !== "rejectedPitch")
    );
  };
