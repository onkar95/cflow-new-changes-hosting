...
function Cart({theme, setCurrentSection,cart,setCart,getCart,site,getAllVendor,currentSection,setIsUser,setCurrentSectionProfile}) {
...
const handleEdit=(e) => {
  ...
  return (
    ...
    <>
    <div className="Cart" style={{backgroundColor:theme?"white":"#121417", color:theme?"black":"white", marginBottom:"5%"}}>
      ...
      <div style={{ display:'flex'}}>
        ...
        <div style={{paddingTop:'10px',padding:'3%',marginLeft:'30%',fontSize:'15px',color: theme?"black":'rgba(245, 245, 245, 0.75)'}}>
          19 March, 2021
        </div>
      </div>
      ...
      <div style={{display:'flex'}}>
        <div>      
          ...
          <div style={{width:'250%'}}>
            {products?.length>0 && products?.map((products,index) => (<div  onClick={()=>setopen(true)}  style={{width:'100%'}}><ProductCard theme={theme} setEdit={setEdit} setCurrentItem={setCurrentItem} products={products} index={index} getCart={getCart}> </ProductCard></div>))}
          </div>
